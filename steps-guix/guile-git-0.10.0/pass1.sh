...
src_configure() {
    local host_triplet pkg_config_path libgit2_cflags libgit2_static_libs
    ...
    libgit2_static_libs="$(PKG_CONFIG_LIBDIR="${pkg_config_path}" PKG_CONFIG_PATH="${pkg_config_path}" \
        /usr/bin/pkg-config --static --libs libgit2)"
    ...
    LIBS="${libgit2_static_libs}" \
    LIBGIT2_CFLAGS="${libgit2_cflags}" \
    LIBGIT2_LIBS="${libgit2_static_libs}" \
    libgit2_CFLAGS="${libgit2_cflags}" \
    libgit2_LIBS="${libgit2_static_libs}" \
...
src_compile() {
    local pkg_config_path libgit2_cflags libgit2_static_libs
